        ...
        <el-table-column label="属性值">
          ...
          <template #default="{ row, $index }">
            <el-input
              ...
              :ref="(inupt: any) => (inputArr[$index] = inupt)"
            ></el-input>
...
// 定义变量控制添加属性界面是否展示,定义card组件内容切换变量
...
//添加属性按钮回调
const addAttr = () => {
  //每一次点击的时候,先清空一下数据再收集数据，主要收集三级分类的ID
...
const check = (len: number) => {
  //非法情况判断1
  if (attrParams.attrValueList[len - 1].valueName.trim() == '') {
    //删除调用对应属性值为空的元素
    attrParams.attrValueList.splice(len - 1, 1)
    //提示信息
    ElMessage({
      type: 'error',
      message: '属性值不能为空',
...
const addAttrValue = () => {
  // 校验如果是
  const len = attrParams.attrValueList.length
  if (len >= 1) {
    //非法情况判断1
    check(len)
  }
  ...
  nextTick(() => {
    inputArr.value[len].focus()
  })
}
//保存属性按钮回调
const save = async (attrParams: any) => {
  //发请求
